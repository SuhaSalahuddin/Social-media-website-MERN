      ...
      Post.findById(req.params.id)
        .then(post => {
          if (
            ...
            return res
              .status(400)
              .json({ alreadyliked: "user already liked this post once" });
          }
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      // Check comments
      const filterRequiredCommentData = post.comments.filter(comment => {
        return comment._id.toString() === comment_id;
      });
      if (!filterRequiredCommentData.length > 0) {
        return res.status(404).json({ message: "Comment does not exists" });
      }

      // Remove comment
      await post.comments.remove(comment_id);
      await post.save();
